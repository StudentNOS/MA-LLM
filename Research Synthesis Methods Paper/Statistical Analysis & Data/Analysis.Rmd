...
# Table 1
```{r}
outcomes <- c("recall", "precision", "F1_score", "specificity", "accuracy")
...
# Table 3
```{r}
vars <- c("F1_score", "recall", "precision", "specificity", "accuracy")
...
Table 4
```{r}
metrics <- c("F1_score","recall","precision", "specificity", "accuracy")
...
result <- result[order(-as.numeric(gsub("\\s*\\(.*\\)$", "", result$F1_score))), ]
